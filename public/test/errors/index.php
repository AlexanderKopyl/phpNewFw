...
class ErrorHandler {
    ...
    public function errorHandler($errno, $errstr, $errfile, $errline){
        error_log("[".date('Y-m-d H:i:s')."] Текст ошибки: {$errstr} Файл: {$errfile} Cтрока: {$errline} \n=====================\n",3,__DIR__ . '/errors.log');
    ...
    public function fatalErrorHandler(){
        ...
        if ( !empty($error) && $error['type'] & ( E_ERROR | E_PARSE | E_COMPILE_ERROR | E_CORE_ERROR))
        {
            ...
            error_log("[".date('Y-m-d H:i:s')."] Текст ошибки: {$error['message']} Файл: {$error['file']} Cтрока: {$error['line']} \n=====================\n",3,__DIR__ . '/errors.log');
    ...
    public function exceptionHandler(Exception $e){
        error_log("[".date('Y-m-d H:i:s')."] Текст ошибки: {$e->getMessage()} Файл: {$e->getFile()} Cтрока: {$e->getLine()} \n=====================\n",3,__DIR__ . '/errors.log');
